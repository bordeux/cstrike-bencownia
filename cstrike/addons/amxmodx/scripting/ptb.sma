...
actAtEndOfRound(){
	...
	say("Auto balance: Runda zakonczona, sprawdzam druzyny")
	checkTeamBalance()
	if (winnerTeam) {
		sortTeam(CTS)
		sortTeam(TS)
...
doSwitch() {
	...
	if ( teamCounts[winnerTeam] == 0 || teamCounts[loserTeam] == 0 ) {
		copy(text,255, "Team balance: Nie mozna zmieniac graczy, potrzebni są gracze w kzżdej druzynie.")
	...
	if (teamCounts[winnerTeam] == 1) {
		copy(text,255, "Team balance: Nie bedzie zmiany graczy, najlepszy gracz tworzy zwyciaska druzyne.")
		doTypesay(text, 5, 0, 255, 0)
		say(text)
		return
	}
	...
	if (teamCounts[winnerTeam] >= PTB_MAXSIZE && teamCounts[loserTeam] >= PTB_MAXSIZE) {
		copy(text,255, "Team balance: Nie mozna zmieniac graczy, obie druzyny są pelne.")
		doTypesay(text, 5, 0, 255, 0)
		say(text)
		return
	}
	if (!PTB_DEADONLY || couldNotSwitchCounter > PTB_FORCESWITCH) {
		...
		if (validTargetCounts[winnerTeam] == 0 || validTargetCounts[loserTeam] == 0) {
			...
			copy(text,255, "Team balance: Nie mozna zmieniać graczy, kazdy team musi miec ustalony cel.")
			doTypesay(text, 5, 0, 255, 0)
			say(text)
			return
		}
		...
		if (validTargetCounts[winnerTeam] == 0 || validTargetCounts[loserTeam] == 0) {
			if (++couldNotSwitchCounter > PTB_FORCESWITCH) {
				say("Team balance: Mozna zmieniac tylko zywych.")
				doSwitch()
				return
			}
			copy(text, 255,"Team balance: Nie mozna zmienic graczy potrzebny cel w kazdej druzynie.")
			doTypesay(text, 5, 0, 255, 0)
			say(text)
			return
		}
	...
	if (winner == 0 && loser == 0) {
		copy(text, 255,"Team balance: Zadna zmiana nie pomoze balansowi.")
		doTypesay(text, 5, 0, 255, 0)
		say(text)
		return
	}
	...
	format(text,255,"Team balance: Zamieniam %s z %s.",winnerName,loserName)
	...
	//say(text)
		...
		client_print(0,print_chat,"Team balance: Zamieniam %s z %s.",winnerName,loserName)
	}else{
		doTypesay(text, 5, 0, 255, 0)
		client_print(0,print_chat,"Team balance: Zamieniam %s z %s.",winnerName,loserName)
		//say(text)
...
doTransfer() {
	...
	if (teamCounts[winnerTeam] == 0) {
			copy(text,255, "Team balance: Nie mozna zamienic graczy, gracze sa potrzebni w kazdym teamie.")
			doTypesay(text, 5, 0, 255, 0)
			say(text)
			return
	}
	if (teamCounts[loserTeam] >= PTB_MAXSIZE) {
		copy(text,255, "Team balance: Nie mozna przeniesc gracza, przegrywajaca druzyna jest pelna.")
		doTypesay(text, 5, 0, 255, 0)
		say(text)
		return
	}
	if (!PTB_DEADONLY || couldNotSwitchCounter > PTB_FORCESWITCH) {
		...
		if (validTargetCounts[winnerTeam] == 0) {
			copy(text,255, "Team balance: Nie mozna przeniesc gracza, brak celu w wygrywajacej druzynie.")
			doTypesay(text, 5, 0, 255, 0)
			say(text)
			++couldNotSwitchCounter
			return
		...
		if (validTargetCounts[winnerTeam] == 0) {
			if (++couldNotSwitchCounter > PTB_FORCESWITCH) {
				say("Team balance: Nie mozna przenosic martwych, przenosze zywych.")
				doTransfer()
				return
			}
			copy(text,255, "Team balance: Nie moge przeniesc gracza, brak prawidlowego celu w wygrywajacych.")
			doTypesay(text, 5, 0, 255, 0)
			say(text)
			return
		}
	...
	if (winner == 0) {
		copy(text, 255,"Team balance: Zaden transfer nie pomoze balansowi.")
		doTypesay(text, 5, 0, 255, 0)
		say(text)
		return
	}
	...
	get_user_name(winner,winnerName,31)
	transferPlayer(winner)
	format(text,255,"Team balance: Przenosze %s do %s",winnerName, (winnerTeam == CTS) ? "Ts" : "CTs")
	...
	//say(text)
		...
		set_hudmessage(0, 255, 0, 0.05, 0.25, 0, 6.0, 5.0 , 0.5, 0.15, -1)
		ShowSyncHudMsg(0, g_MyMsgSync, "%s", text)
		client_print(0,print_chat,"Team balance: Przenosze %s do %s",winnerName, (winnerTeam == CTS) ? "Ts" : "CTs")
	}else{
		doTypesay(text, 5, 0, 255, 0)
		client_print(0,print_chat,"Team balance: Przenosze %s do %s",winnerName, (winnerTeam == CTS) ? "Ts" : "CTs")
		//say(text)
...
checkTeamSwitch(id,iNewTeam) {
	...
	if (iNewTeam==CTS||iNewTeam==TS){
		...
		if (teamCounts[iNewTeam] && teamCounts[opposingTeam]<PTB_MAXSIZE &&
			...
			get_user_name(id,name,31)
			if (++wtjCount[id] >= PTB_WTJKICK && PTB_KICK) {
				format(text, 255, "Team balance: Wyrzucam %s za WTJ ilosc %d of %d.", name, wtjCount[id],PTB_WTJKICK)
				doTypesay(text, 5, 0, 255, 0)
				say(text)
				server_cmd("kick #%d", get_user_userid(id))
				return PLUGIN_HANDLED
			...
			if (iNewTeam==CTS) {
				if (wtjCount[id]>=PTB_WTJAUTO && is_user_connected(id)) {
					manageWtjFile(id)
					format(text, 255, "Team balance: Wymuszam %s do Ts (WTJ: %d/%d).", name, wtjCount[id],PTB_WTJKICK)
					...
					format(text, 255, "Team balance: CT jest silne wystarczajaco, %s (WTJ: %d/%d).", name, wtjCount[id],PTB_WTJKICK)
					doTypesay(text, 5, 0, 50, 255)
					say(text)
#if !defined MANUAL_SWITCH
					engclient_cmd(id,"chooseteam") // display menu again
				...
				if (wtjCount[id]>=PTB_WTJAUTO) {
					manageWtjFile(id)
					format(text, 255, "Team balance: Wymuszam s do CTs (WTJ: %d/%d).", name, wtjCount[id],PTB_WTJKICK)
					...
					format(text, 255, "Team balance: Ts sa silni dostatecznie, %s (WTJ: %d/%d).", name, wtjCount[id],PTB_WTJKICK)
					doTypesay(text, 5, 255, 50, 0)
					say(text)
#if !defined MANUAL_SWITCH
					engclient_cmd(id,"chooseteam") // display menu again
...
announceStatus() {
	...
	new text[256]
	if (winnerTeam == TS) {
		format(text, 255, "Team balance: CT przydaloby sie wsparcie.")
		doTypesay(text, 5, 0, 50, 255)
		say("Team balance: CT przydaloby sie wsparcie.")
	}
	else if (winnerTeam == CTS) {
		format(text, 255, "Team balance: T przydaloby sie wsparcie.")
		doTypesay(text, 5, 255, 50, 0)
		say("Team balance: T przydaloby sie wsparcie")
	}
	else if (wtConditions[TS] > wtConditions[CTS]) {
		format(text, 255, "Team balance: Obserwuje przewage TERRO.")
		doTypesay(text, 5, 255, 50, 0)
		say("Team balance: Obserwuje przewage TERRO.")
	}
	else if (wtConditions[CTS] > wtConditions[TS]) {
		format(text, 255, "Team balance: Obserwuje przewage CT.")
		doTypesay(text, 5, 0, 50, 255)
		say("Team balance: Obserwuje przewage CT.")
	}
	else if (PTB_SAYOK) {
		format(text, 255, "Team balance: Balans wyglada ok.")
		doTypesay(text, 5, 200, 100, 0)
		say("Team balance: Balans wyglada ok.")
	}
}
