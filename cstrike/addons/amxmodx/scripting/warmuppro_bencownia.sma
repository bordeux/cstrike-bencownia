...
{	
	...
	client_print(plr, print_console, "Rozgrzewka wylaczona")
	ColorChat(0, GREEN,"^x04Admin wylaczyl rozgrzewke")
...
public admin_exec2(plr, level)
{
	...
	ColorChat(0, GREEN,"^x04Admin wlaczyl rozgrzewke")
...
public darresultado()
{
	...
	if( resultadofinal >= 1 )
	{
		if( !g_muchos )
		{
			ColorChat(0, GREEN, "^x04Rozgrzewke wygral:^x03 %s^x04 z^x03 %d^x04 fragami", name, resultadofinal)
	...
	if( resultadofinal == 0 )
	{
		ColorChat(0, GREEN, "Nikt nie wygral rozgrewki")
...
public muza_on()
{
	...
	client_cmd(0,"mp3 play sound/szczurownia/rozgrzewka.mp3");
}
